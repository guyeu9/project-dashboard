...
import type { PMO, InsertPmo, UpdatePmo } from "./shared/schema";

export class PmoManager {
  async createPmo(data: InsertPmo): Promise<PMO> {
  ...
  async getPmoById(id: string): Promise<PMO | null> {
    const db = await getDb();
    const [pmo] = await db.select().from(pmos).where(eq(pmos.id, id));
    return pmo || null;
  }

  async updatePmo(id: string, data: UpdatePmo): Promise<PMO | null> {
    const db = await getDb();
  ...
  async deletePmo(id: string): Promise<boolean> {
    const db = await getDb();
    const result = await db.delete(pmos).where(eq(pmos.id, id));
    return (result.rowCount ?? 0) > 0;
  }

  async getPmoOptions(): Promise<Array<{ id: string; name: string }>> {
    const db = await getDb();
    return db
      .select({
        id: pmos.id,
