...
// 产品经理表
export const productManagers = pgTable(
  "product_managers",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    name: varchar("name", { length: 100 }).notNull(),
...
export const projects = pgTable(
  "projects",
  {
    ...
    partners: jsonb("partners").$type<string[]>().default(sql`'[]'::jsonb`).notNull(),
    developers: jsonb("developers").$type<string[]>().default(sql`'[]'::jsonb`).notNull(),
    testers: jsonb("testers").$type<string[]>().default(sql`'[]'::jsonb`).notNull(),
    owner: varchar("owner", { length: 100 }),
    productManager: varchar("product_manager", { length: 36 }),
    pmo: varchar("pmo", { length: 36 }),
    remark: text("remark"),
    chatGroupLinks: jsonb("chat_group_links").$type<string[]>().default(sql`'[]'::jsonb`).notNull(),
    contacts: jsonb("contacts").notNull().$type<Array<{ name: string; role: string; phone: string }>>().default(sql`'[]'::jsonb`),
    dailyProgress: jsonb("daily_progress").notNull().$type<Array<{
      date: string;
      progress: number;
...
export const tasks = pgTable(
  "tasks",
  {
    ...
    startDate: timestamp("start_date", { withTimezone: true }),
    endDate: timestamp("end_date", { withTimezone: true }),
    assignees: jsonb("assignees").$type<string[]>().default(sql`'[]'::jsonb`).notNull(),
    dailyProgress: text("daily_progress"),
    remark: text("remark"),
    dailyRecords: jsonb("daily_records").notNull().$type<Array<{
      date: string;
      progress: number;
      status: 'normal' | 'risk' | 'delayed';
      content: string;
